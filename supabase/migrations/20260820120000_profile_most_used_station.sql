alter table public.profile_stats
  add column if not exists most_used_station_name text;

create or replace function public.bicimad_public_station_code(
  p_station_id text,
  p_station_name text
)
returns text
...
as $$
  ...
  select case
    ...
    else (parts[1]::bigint)::text || lower(coalesce(parts[2], ''))
  end
  from matches;
$$;
...
declare
  ...
  most_used_station_name_value text;
begin
  ...
  with endpoints as (
    select
      public.bicimad_public_station_code(
        j.origin_station_id,
        j.origin_station_name
      ) as public_code,
      j.origin_station_id as station_id,
      public.bicimad_station_display_name(
        j.origin_station_id,
        j.origin_station_name
      ) as station_name,
      j.started_at as used_at
    from pg_temp.canonical_profile_journeys j
    union all
    select
      public.bicimad_public_station_code(
    ...
    select
      coalesce(
        nullif(public_code, ''),
        'name:' || nullif(
          lower(regexp_replace(coalesce(station_name, ''), '[^[:alnum:]]+', '', 'g')),
          ''
        ),
        'id:' || nullif(btrim(station_id), '')
      ) as station_key,
      station_name,
      used_at
    from endpoints
    ...
    select
      station_key,
      (array_agg(station_name order by used_at desc))[1] as station_name,
      count(*) as uses,
      max(used_at) as latest_use
    from keyed_endpoints
    where station_key is not null and station_name is not null
    group by station_key
  )
  select station_name
  into most_used_station_name_value
  from station_counts
  order by uses desc, latest_use desc, station_key
  limit 1;
  ...
  on conflict (user_id) do update set
    ...
    most_used_station_name = excluded.most_used_station_name,
    updated_at = excluded.updated_at;
end;
$$;

revoke all on function public.refresh_profile_stats(uuid) from public;
revoke all on function public.bicimad_public_station_code(text, text) from public;
revoke all on function public.bicimad_station_display_name(text, text) from public;

drop function if exists public.search_social_profiles(text);

create function public.search_social_profiles(p_query text)
returns table (
  user_id uuid,
  username text,
  display_name text,
  ...
  with visible_profiles as (
    select
      ...
      (
        ...
        or p.is_public
        or exists (
          select 1 from public.follows allowed_follow
          where allowed_follow.follower_id = auth.uid()
            and allowed_follow.following_id = p.user_id
...
drop function if exists public.list_social_connections(text);

create function public.list_social_connections(p_kind text)
returns table (
  user_id uuid,
  username text,
  display_name text,
...
as $$
  with connections as (
    select
      p.*,
      f.status as connection_status,
      f.created_at as connection_created_at,
      case when f.follower_id = auth.uid() then 'outgoing' else 'incoming' end
        as connection_direction,
      (
        p.user_id = auth.uid()
        or p.is_public
        or exists (
  ...
  select
    ...
    p.connection_status,
    p.connection_direction,
    p.connection_created_at,
    case when p.can_view_stats then s.most_used_station_name else null end
  from connections p
  left join public.profile_stats s on s.user_id = p.user_id
  left join public.follows outgoing
    on outgoing.follower_id = auth.uid() and outgoing.following_id = p.user_id
  order by p.connection_created_at desc;
$$;
